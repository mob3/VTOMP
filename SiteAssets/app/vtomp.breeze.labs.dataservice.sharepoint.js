...
(function (definition) {
  if (typeof breeze === "object") {
    ...
    // Requirejs / AMD
    define([//'./crud/crudstate',
  //  './fieldDict',
    'breeze', "breeze.labs.dataservice.abstractrest"], definition);
  } else {
...
breeze) {
  ...
  function _createChangeRequest(saveContext, entity, index) {
    ...
    if (state.isAdded()) {
      ...
      $.each(entity.entityAspect.entityGroup.entityType.dataProperties, function(key, val) {
     	if (!val.isScalar && !val.isComplexProperty) { //match only the arrays
     		delete rawEntity[val.name]; //fix this up
      		if (entity.getProperty(val.name).length) {
				rawEntity[val.name] = {
						"__metadata": { type: "Collection(Edm.String)" },
						results: entity.getProperty(val.name)
				};
      		}
      	}
      });
    ...
    //  loop and find where isScalar is false and not is complex 
      $.each(entity.entityAspect.entityGroup.entityType.dataProperties, function(key, val) {
//       then if entity.getProperty("DHSComponentExperience")._origValues !arrayEquals(entity.getProperty("DHSComponentExperience"))
     	if (!val.isScalar && !val.isComplexProperty) { //match only the arrays
//      $.arrayCompare(entity.getProperty("DHSComponentExperience"), entity.getProperty("DHSComponentExperience")._origValues)
      		if (!$.arrayCompare(entity.getProperty(val.name), entity.getProperty(val.name)._origValues)) {
 //			     add to rawEntity
				rawEntity[val.name] = {
						"__metadata": { type: "Collection(Edm.String)" },
						results: entity.getProperty(val.name)
				};
				delete entity.getProperty(val.name)._origValues;
		/*		origVals.length = 0;
				$.each(entity.getProperty(val.name), function(key, val) {
					origVals.push(val);
				}); */
      		}
      	}
    ...
    function updateEntityNode(node, mappingContext, result) {
      ...
      if (entityType) {
        ...
        if (entityType._mappedPropertiesCount <= Object.keys(node).length - 1) {
          ...
          // fix arrays based on fields that allow multiple choice
          var arrayPropNames = _.map(_.where(entityType.dataProperties, {isScalar : false}), function(dp) { return dp.name; });
          arrayPropNames.forEach(function (pn) { 
          	if (node[pn] && node[pn].results && node[pn].__metadata && Array.isArray(node[pn].results)) {
          		node[pn] = node[pn].results;
          	}
          });
        }
      }
  ...
  function _getResponseData(response) {
    ...
    //	var listName = serverTypeNameToClientDefault(JSON.parse(response.config.data).__metadata.type);
   // 	crudstate.currentListEtags[listName ] = etag;
    	return null; // 	var results = [];
   // 	results.push({__metadata : { etag : etag }});
   // 	return results;
    }
  }
